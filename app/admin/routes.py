...
def show_tutors():
    ...
    if q_first:
        ...
        tutors = db.session.query(models.User).join(models.Tutoring).filter(and_(models.Tutoring.tutor_subjects.any(subject=q_subject), models.Tutoring.tutor_modalities.any(modality=q_modality)))
    else:
