...
Revision ID: 01788eaebc69
Revises: 31db25d0a8cb
Create Date: 2022-08-08 20:15:57.346818
...
revision = '01788eaebc69'
down_revision = '31db25d0a8cb'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    sa.ForeignKeyConstraint(['upload_id'], ['myupload.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_myuser_email'), 'myuser', ['email'], unique=True)
    op.create_index(op.f('ix_myuser_username'), 'myuser', ['username'], unique=True)
    ...
    sa.Column('course_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['course_id'], ['course.id'], ),
    sa.ForeignKeyConstraint(['tutor_id'], ['myuser.id'], )
    )
...
def downgrade():
    ...
    op.drop_index(op.f('ix_myuser_username'), table_name='myuser')
    op.drop_index(op.f('ix_myuser_email'), table_name='myuser')
    op.drop_table('myuser')
    op.drop_table('myupload')
